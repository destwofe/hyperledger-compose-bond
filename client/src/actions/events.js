...
export const fetchMoneyTransferEvents = (moneyWalletId) => (dispatch) =>
  Axios.get(`${BASE_URL}/api/events/moneytransfer?moneyWallet=${moneyWalletId}&resolve=true`)
    .then(response => {
      dispatch({
...
export const fetchBondTransferEvents = (bondWalletId) => async (dispatch) =>
  Axios.get(`${BASE_URL}/api/events/bondtransfer?bondWallet=${bondWalletId}&resolve=true`)
    .then(response => {
