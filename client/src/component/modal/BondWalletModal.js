...
class BondWalletModal extends Component {
  ...
  fetchBonds = () => {
    Axios.get('http://api.destwofe.zyx/api/bonds', { headers: { accessToken: this.state.accessToken } })
      .then((response) => {
  ...
  postCreateBondWallet = () => {
    Axios.post('http://api.destwofe.zyx/api/bondwallets', { ...this.state.bondWallet }, { headers: { accessToken: this.state.accessToken } })
      .then((response) => {
