...
class IssueBondModal extends Component {
  ...
  postIssueBond = () => {
    ...
    if (bondObject.issuerMoneyWallet !== 'select' && bondObject.symbole !== '') {
      Axios.post('http://api.destwofe.xyz/api/bonds', bondObject, { headers: { accessToken: this.state.accessToken } })
        .then((response) => {
    ...
    //   Axios.post('http://api.destwofe.xyz/api/transaction/bondtransfer', transferTransaction, { headers: { accessToken: this.state.accessToken } })
    //     .then((response) => {
    //       this.props.toggle(true)
    //     })
    //     .catch(error => NotificationManager.error(error.toString, 'Uncatch exception'))
