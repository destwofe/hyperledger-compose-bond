      ...
      <Modal open={this.props.isOpen} onClose={() => this.props.setModalOpenName(null)}>
        <Paper style={{ position: 'absolute', top: '45%', left: '50%', transform: `translate(-${45}%, -${50}%)`, padding: 20 }}>
          <Grid container justify="center">
            ...
            <Grid item sm={9} style={{ paddingTop: 15 }}><TextField fullWidth type="number" defaultValue={this.state.hardCap} label="Sale Amount" helperText={`(unit) [${Number(this.state.hardCap*this.state.parValue).toLocaleString()} THB]`} onChange={(event) => this.setState({ hardCap: Number(event.target.value) })} /></Grid>
            <Grid item sm={9} style={{ paddingTop: 15, paddingBottom: 20 }}><TextField fullWidth select label="Money Receive: Bank account" value={this.state.issuerMoneyWallet} helperText={`balance ${getSafe(() => Number(this.props.moneyWallets.find(a => a.id === this.state.issuerMoneyWallet).balance).toLocaleString()) || 0} THB`} onChange={(event) => this.setState({ issuerMoneyWallet: event.target.value })}>{this.props.moneyWallets.map(moneyWallet => <MenuItem key={moneyWallet.id} value={moneyWallet.id}>{`${moneyWallet.id}`}</MenuItem>)}</TextField></Grid>
          </Grid>
