...
import { moneyDepositTransaction } from '../../actions/transaction';

export const NAME = 'MONEY_DEPOSIT'
export default connect((state) => ({ isOpen: state.view.modalOpenName === NAME, moneyWallet: state.asset.moneyWallets.find((a) => a.id === state.view.moneyWalletSelectedId) || state.asset.moneyWallets[0] }), { setModalOpenName, moneyDepositTransaction })(class extends Component {
  state = {
    to: '',
    amount: 0,
    isLoading: false
  ...
  submitHandler = () => {
    ...
    const data = {
      to: this.props.moneyWallet.id,
      amount: this.state.amount
    }
    this.props.moneyDepositTransaction(data)
      .then(response => {
      ...
      <Modal open={this.props.isOpen} onClose={() => this.props.setModalOpenName(null)}>
        <Paper style={{ position: 'absolute', top: '45%', left: '50%', transform: `translate(-${45}%, -${50}%)`, padding: 20 }}>
          <Grid container justify="center">
            <Grid item sm={12}><Typography variant="title">Money Deposit</Typography></Grid>
            <Grid item sm={9} style={{ paddingTop: 15 }}><TextField fullWidth label="To" disabled value={this.props.moneyWallet.id} /></Grid>
